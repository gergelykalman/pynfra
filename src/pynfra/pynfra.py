...
from pynfra.modules.config import CONFIG
from pynfra.modules.db import DB
from pynfra.modules.ssh_keys import generate_key, remove_hostkeys
from pynfra.modules.node import Node, NodeAuthException, NodeTimeoutException
from pynfra.modules.tui import TUI
...
class Pynfra:
    def __init__(self):
        ...
        # parse arguments
        self.__parser = None
        self.__args, self.__unknownargs = self.__parse_args()

        # config paths
        self.__CONF_DIR = self.__args.confdir
        self.__CONFIGNAME = os.path.join(self.__CONF_DIR, "pynfra.json")
        self.__HOSTKEYS = os.path.join(self.__CONF_DIR, "hostkeys")
        self.__PRIVPATH = os.path.join(self.__CONF_DIR, "pynfra_key")
        self.__PUBPATH = os.path.join(self.__CONF_DIR, "pynfra_key.pub")
    ...
    def need_init(f, *args, **kwargs):  # *args, **kwargs makes pycharm shut up
        def magic(self, *args, **kwargs):
            if self.__first_run:
                print("Pynfra has not been initialized, run pynfra init YOUR_EMAIL_ADDR")
                exit(1)
            return f(self, *args, **kwargs)
        return magic

    def load_recipe(self, name, recipe):
        if not issubclass(recipe, RecipeBase):
            print("[-] Recipe {} has to be of type {}".format(recipe, RecipeBase))
            exit(1)
...
#                }
    ...
    def init(self, email):
        ...
        if not self.__first_run:
            print("[-] Pynfra init already ran, if you want to re-run init, delete the {} directory".format(
                self.__CONF_DIR))
            exit(1)
    ...
    @need_init
    def recipe(self, recipename, nodename, *args):
        if recipename not in self.__recipes:
            print("Recipe \"{}\" not found, available recipes:".format(recipename))
            for name, recipe in self.__recipes.items():
                print("\t{}".format(name))
                exit(1)

        node = self.__get_server(nodename)

        recipe = self.__recipes[recipename](node)
        recipe.main(args)

    def __parse_args(self):
        # if --confdir is set use it
        # if not and CONFDIR is set in env, use it
        # if not fall back to $(pwd)/.pynfra
        confdir_default = os.environ.get(
            "CONFDIR",
            os.path.join(self.__CWD, ".pynfra")
        )
        ...
        self.__parser.add_argument('--confdir', type=str, required=False,
                                   ...
                                   help='Set pynfra config directory')
        ...
        parser_add = subparsers.add_parser('add', help='add node to pynfra')
        ...
        parser_add = subparsers.add_parser('remove', help='remove node from pynfra')
        parser_add.add_argument('hostname', type=str, help='hostname')
        ...
        args, unkownargs = self.__parser.parse_known_args()
        return args, unkownargs
